...
import (
	...
	"golang.org/x/net/publicsuffix" // <-- ADD THIS IMPORT
...
func (c *Crawler) extractWhois(ctx context.Context, domain string, nf *config.NodeFeatures) (err error) {
	// --- MODIFIED: Use the apex domain for WHOIS lookups ---
	...
	// NEW: Add a defer function to recover from panics in the parser library.
	defer func() {
		if r := recover(); r != nil {
...
// *** to handle the official PhishTank CSV format ***
