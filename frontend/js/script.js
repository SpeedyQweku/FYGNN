document.addEventListener('DOMContentLoaded', () => {
    ...
    function initPredictPage() {
        ...
        const workersInput = document.getElementById('workers-predict'); // Get the new input

        enableParamsCheck.addEventListener('change', () => {
            const isDisabled = !enableParamsCheck.checked;
            paramsContainer.classList.toggle('disabled', isDisabled);
            depthInput.disabled = isDisabled;
            workersInput.disabled = isDisabled; // Add to the enable/disable logic
        });
    ...
    function addTextToTerminal(terminalEl, text) {
        ...
        if (isProgressBar) {
            ...
            // If the last line was also a progress bar, replace its content.
            // Otherwise, create a new line for the progress bar.
            if (lastLineElement && lastLineElement.textContent.includes('%|')) {
                lastLineElement.textContent = text.trim(); // Use trim() to remove extra whitespace
            } else {
            ...
            // This is for normal log lines, so we create a new paragraph for each.
            const p = document.createElement('p');
            p.textContent = text;
            if (cursor) {
                cursor.before(p);
